...
def load_gray_scale_tensor_cv(im_path, device, imsize=None, dfactor=1, scale_type=min):
    # Used for LoFTR
    im = cv2.imread(im_path, cv2.IMREAD_GRAYSCALE)
    ho, wo = im.shape
    wt, ht, scale = resize_im(wo, ho, imsize=imsize, dfactor=dfactor, value_to_scale=scale_type)
